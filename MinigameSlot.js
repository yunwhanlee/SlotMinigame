...
//* AUTO当たりカウントの周期
let WIN_MODE_SPAN = 7; // この倍数になったら、当たりモードON
...
//* エフェクトFPSのオプション
let isHalfFpsOpt = true; // 実際のサーバでフリーズ掛かったので、FPSを60 -> 30に落とすオプション用意。
let gamePadReqAnim = null;
let gamePadUpdate = null;

//* ゲームパットのスピンリール押したトリガー
...
let canvasHeight = window.innerHeight; //* 縦の基準で、幅の比率を合わせる
let canvasWidth = canvasHeight * W_RATIO_BY_H;
setCanvasAspect(); //* Canvasの比率を合わせる計算。
...
const STOPSPIN_TIME = 1500; // ストップ時間
const WAIT_NEXT_CLICK_TIME = STOPSPIN_TIME * 0.8;
const FIXPOS_TIME = 400; // LongTypeの位置揃え時間
const CONGRETURATION_TIME = 4000;
const AUTO_CLICK_TIME = 10000; // 1000 -> 1秒
...
//* --------------------------------------------------------------------------------------------
    ...
    gamePadUpdate = () => {
        ...
        if (buttonPressed(gp.buttons[OPTIONS_BTN])) {
            ...
            //* reset()内に pressCntが有りますが、ゲームパッドの場合、いつでもpressCntが増えるようにして、途中でも当たりモードに入りようにする。
            else {
                pressCnt++;
                setWinModeByPressCnt();
            }
        }
        //* OPTIONボタンが離れたら、pressCntを0にする。
    ...
    //! GamePadが繋がったら、毎回 OPTIONボタンのpressedがfalseのため、pressCntが０になる。
    //? PCでプレイしたいなら、GamePadを外してください。
    //* スペースバーを押す
    window.addEventListener('keydown', (e) => {
...
//* --------------------------------------------------------------------------------------------
    ...
    onClickStopSpinBtn = () => {
        // spin1Txt.visible = false;
        //* Credet(ハート)が無かったら、以下を実行しない。
        // if(heartCnt < 0) {
        ...
        if(!reel.isStopTrg && !reel2.isStopTrg && !reel3.isStopTrg
            ...
            //* プレイカウントで当たりモードの周期になったら、WinモードON
            ...
            //* WinモードがONの場合、モデルIndexをランダムで切り替える。（続いてプレイしても、自然な動きをするため）
            if(isWinControlTrigger) {
        ...
        if(!reel3.isStopTrg) {
            ...
            setTimeout(() => {
                ...
                focusArrow.x = 9999;
                focusArrow.y = 9999;
            }, WAIT_NEXT_CLICK_TIME * 2); // 1秒後で行う : tweenToのbackout(0.5)エフェクトが最後に適用できないバグ対応
            console.log("onClickStopSpinBtn():: After Trigger\nr1.Trg:", reel.isStopTrg, "r2.Trg:", reel2.isStopTrg, "r3.Trg:", reel3.isStopTrg, "\nr1.Run:", reel.isRun, "r2.Run:", reel2.isRun, "r3.Run:", reel3.isRun);
            return;
        }
    }
            ...
            setTimeout(() => {
                ...
                r.symbols.forEach(symbol => {
                    ...
                    if(ResultSymbolRangeMinY < symbol.y && symbol.y <= ResultSymbolRangeMaxY) {
                        //* 止まったシンボルを確認。
                        // symbol.tint = 0xff0000; // Debug用の赤色
        ...
        setTimeout(() => {
            ...
            resList.forEach((res, i) => {
                if((firstIdx + i) != res.idx)
                    resList.unshift(resList.pop()); //* 末端のIDX➝初端に移動。
            });

            // resList.forEach(res => logTxt.text += `\n[${res.idx}] ${res.name}`);

            //* 合わせてLongTypeSymbolの位置調整
            const longType = {
                isOn: false,
                name: ""
            }
    ...
    function updateReel(r, slotMassCnt) {
        ...
        for (let j = 0; j < r.symbols.length; j++) {
            ...
            if (s.y < 0 && prevy > SYMBOL_SIZE) {
                ...
                //* ノーマルシンボルの出る確率(N割り)
                const WIN_MODE_NORMAL_PER = 6; //* 当たりモードなら
                const GENERAL_NORMAL_PER = 5; //* 普通なら
                ...
                //* 絶対当たるモードONなら、ランダムで選択したモデルIDXに固定。
                ...
                if (isLongType) {
                    ...
                    s.alpha = 0; //* LongTypeと重なるSymbolイメージを非表示。
...
// #endregion
    ...
    //* LONGタイプならslot1MassCntが2になり、シンボルが重ならないように非表示する。
    let slot1MassCnt = 0;
    let slot2MassCnt = 0;
    let slot3MassCnt = 0;
    let randIdxList = [];
...
function updateEF(intervalID, imgElem, efPaths) {
    ...
    const speed = isHalfFpsOpt? 40 : 30; // 再生速度
    //* ID 初期化
    if(intervalID) clearInterval(intervalID); // まだ終わらなかったら、以前のことクリアー
    else setTimeout(() => clearInterval(intervalID), speed * FRAME_CNT); // スタート
    let i = 0;
...
function setCanvasAspect() {
    ...
    if(canvasWidth > window.innerWidth) {
        ...
        const ratio = window.innerWidth / canvasWidth; //* Windowの幅をcanvas幅にに分けて、幅が全て見える比率を計算。
        canvasHeight *= ratio; //* 基準になる縦に比率を掛けて
        canvasWidth = canvasHeight * W_RATIO_BY_H; //* 合う比率を再計算及び代入
    }
}
